...
// Initialize Tauri APIs if available (Tauri v2 uses plugin-fs and plugin-dialog)
const initTauriApis = async () => {
    if (typeof window !== 'undefined' && (window as any).__TAURI__) {
        try {
            const importModule = (p: string) => import(p)
            const fs = await importModule('@tauri-apps/plugin-fs').catch(() => null)
            const dialog = await importModule('@tauri-apps/plugin-dialog').catch(() => null)
...
export const saveExcelFileWithSheetsNative = async (
    ...
    try {
        ...
        if (tauriApis && outputPath) {
            // Save to specific path using Tauri fs plugin (writeFile for binary)
            await tauriApis.fs.writeFile(outputPath, data)
            return outputPath
        } else if (tauriApis) {
            // Use save dialog
            ...
            if (filePath) {
                await tauriApis.fs.writeFile(filePath, data)
                return filePath
            } else {
...
export const saveExcelFileNative = async (data: any[], filename: string, outputPath?: string): Promise<string> => {
    ...
    try {
        ...
        if (tauriApis && outputPath) {
            // Save to specific path using Tauri fs plugin
            await tauriApis.fs.writeFile(outputPath, binaryData)
            return outputPath
        } else if (tauriApis) {
            // Use save dialog
            const filePath = await tauriApis.dialog.save({
            ...
            if (filePath) {
                await tauriApis.fs.writeFile(filePath, binaryData)
                return filePath
            } else {
                throw new Error('No file path selected')
            }
...
export const readExcelFile = async (file: File, filePath?: string): Promise<any[]> => {
    try {
        ...
        if (filePath) {
            ...
            if (tauriApis) {
                // Use Tauri fs plugin for file reading
                data = await tauriApis.fs.readFile(filePath)
            } else {
